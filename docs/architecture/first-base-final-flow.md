...
4. 회원 생년월일·병역은 [진단용 회원정보와 초깃값 API](plan-profile-prefill.md)로 연결했다. 오픈뱅킹 소득은 [안전한 금융정보 동기화](open-banking-plan-sync.md)로 연결했으며 미확인 외부값은 정책 판정에 쓰지 않는다. 계좌 출금가능액·대출상환액은 참고 정보이고 순자산·가용현금으로 자동 변환하지 않는다. 실수령 추정액은 은행의 증빙 연소득과 같지 않다.
