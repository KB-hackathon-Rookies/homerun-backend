...
1. 정책 규칙 버전 재검수: 일반 버팀목 소득·자산·지역별 보증금/대출한도, 서울시 연령·지역·소득 조건을 최종 플로우에 맞춘 새 버전으로 작성해야 한다. 적용된 Flyway는 수정하지 않는다.
...
4. 회원 생년월일·병역, 오픈뱅킹 조회값의 입력 스냅샷 연계와 수동 수정 출처·확인 상태 검증을 완료해야 한다. 실수령 추정액은 은행의 증빙 연소득과 같지 않다.
...
서울주거포털에서는 2026-06-05 소득기준 상향 안내를 확인했다. 주택도시기금 원문은 이번 조회에서 시간초과가 발생해 새 금융 계산값을 검증한 것으로 취급하지 않았다.
