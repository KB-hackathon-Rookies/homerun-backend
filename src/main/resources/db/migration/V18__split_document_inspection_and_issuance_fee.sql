...
VALUES
  ...
  -- 건축물대장은 등본과 초본이 같은 값이다. 갈리는 것은 발급(500원)과 열람(300원)이다.
  ('FCT-167', '비용', '건축물대장 발급', '방문 발급 건당 500원 / 인터넷 무료', 500.0, '원',
   '등본·초본 동일', '정부24', 'CONFIRMED', '수시', 'ISS-01',
   '열람은 300원(FCT-168). 제출용은 발급이다', '2026-01-01', 'fact-registry'),

  ('FCT-168', '비용', '건축물대장 열람', '열람 건당 300원', 300.0, '원',
   '등본·초본 동일', '정부24', 'CONFIRMED', '수시', 'ISS-01',
   '제출용으로는 안 된다. 발급은 500원(FCT-167)', '2026-01-01', 'fact-registry'),

  -- 무인발급기만 지역마다 다르다. 수치를 두지 않는다. 하나로 박으면 지역에 따라
  -- 틀린 안내가 되고, 확인된 방문 500원까지 provisional 로 끌어내리게 된다.
  ('FCT-169', '비용', '건축물대장 무인 발급', '자치단체 조례에 따라 다름', NULL, NULL,
   '무인민원발급기', '정부24', 'REVIEW', '수시', 'ISS-01',
   '전국 공통값이 없다. 발급기 화면에서 확인해야 한다', '2026-01-01', 'fact-registry');

-- ISS-01 시드의 안내 문구도 등본·초본이 아니라 발급·열람으로 바로잡는다.
UPDATE document_issue_method
SET fee_note = '발급 500원, 열람 300원'
WHERE fee_note = '등본 500원, 초본 300원';
