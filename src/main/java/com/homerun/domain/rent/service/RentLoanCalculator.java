...
public class RentLoanCalculator {
    ...
    /**
     * 두 상품을 비교한다(POL-04-03).
     *
     * <p>총 이자만으로 줄 세우면 안 된다. 두 상품은 빌려주는 원금이 달라서, 보증금이 필요한
     * 사용자에게 보증금을 아예 안 빌려주는 주거안정이 "이자가 싸다"는 이유로 1순위가 될 수
     * 있다. 못 채운 금액은 다른 데서 구해야 하므로 동등 비교가 아니다.
     *
     * <p>그래서 요청 자금을 채우는 상품을 먼저 놓고, 그 안에서 총 이자로 정렬한다.
     */
    public List<RentLoanQuote> compare(RentLoanRequest request) {
        ...
        quotes.sort(Comparator.comparing(RentLoanQuote::coversRequest)
                .reversed()
                .thenComparingLong(RentLoanQuote::totalInterest));
        return List.copyOf(quotes);
    }

    /** 요청한 보증금·월세 중 이 상품이 못 채우는 금액. */
    private long fundingGap(RentLoanRequest request, long depositLoan, long rentLoan, int termMonths) {
        long depositGap = Math.max(0, request.deposit() - depositLoan);
        long rentGap = Math.max(0, request.monthlyRent() - rentLoan) * termMonths;
        return depositGap + rentGap;
    }
    ...
    public RentLoanQuote quoteDepositBacked(RentLoanRequest request) {
        ...
        long gap = fundingGap(request, depositLoan, rentLoan, term);
        if (gap > 0) {
            notes.add("요청 자금 중 %,d원은 이 상품으로 채울 수 없다".formatted(gap));
        }
    ...
    public RentLoanQuote quoteHousingStability(RentLoanRequest request) {
        ...
        long gap = fundingGap(request, 0, rentLoan, request.termMonths());
        if (gap > 0) {
            notes.add("요청 자금 중 %,d원은 이 상품으로 채울 수 없다. 보증금은 따로 마련해야 한다".formatted(gap));
        }
