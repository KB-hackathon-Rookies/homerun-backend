 ...
 *
 * <p>기본값은 {@code KIM_FIRST}(김첫집)다. 시연 대본이 "독립을 준비하는 평범한 청년" 한 명으로
 * 흐르기 때문에, 페르소나를 고른 적 없는 회원 — 가짜 오픈뱅킹 연결만 한 회원 포함 — 은 전부
 * 김첫집으로 보여야 한다. 다른 페르소나는 {@code POST /api/v1/plans/{planId}/input/open-banking-sync/mock}
 * 로 명시적으로 실었을 때만 나온다.
 *
...
public class MockPersonaSelection {
    ...
    public MockPersonaSelection(@Value("${external-api.open-banking.mock-persona:KIM_FIRST}") Persona defaultPersona) {
     ...
     * 답한다 — 데모 화면이 비어 보이는 것보다 김첫집으로 일관되게 보이는 편이 낫다.
     */
