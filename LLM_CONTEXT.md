...
/api/v1/plans/{planId}/first-base/complete STEP 검증·진단 저장·1루 완료 통합 제출
/api/v1/plans/{planId}/first-base/result   완료 당시 정책·진단 시나리오 스냅샷 복원
...
* **청년미래적금은 뺐다.** `POL-02-02`(매칭)과 `AST-01-06`(적금 중도해지 손실 안내) 둘 다 범위 밖이다 — **청년미래적금 제외 결정** 참고
...
**`POL-02-02`(청년미래적금 매칭)와 `AST-01-06`(적금 중도해지 손실 안내)은 구현하지 않는다.** 2026-09-06 결정.

* 시트2에는 둘 다 남아 있다. 시트2를 아직 고치지 않았으므로 여기가 더 최신이다.
* 판정 API(`/policies/youth-savings`)와 서비스·컨트롤러는 지웠다. `AssetType` 에서도 `YOUTH_SAVINGS` 를 뺐다.
* **`policy` 행과 규칙은 지우지 않고 규칙만 `DRAFT` 로 되돌렸다**(`V42`). 팩트(`FCT-179`~`183`, `FCT-083`·`084`)도 남아 있다 — 되살리려면 `V31` 같은 승격 마이그레이션 하나면 된다.
* ⚠️ **`DISCONTINUED` 로 두지 않았다.** 중기청(`FCT-135`)에 그 상태를 쓴 선례가 있지만 그건 실제로 신규 취급이 중단된 상품이다. 청년미래적금은 우리가 범위에서 빼는 것뿐이라 폐지로 표시하면 "이제 없는 상품"이라는 거짓 안내가 된다. `ADM-07` 의 폐지 상태는 실제 폐지 상품에만 쓴다.
* **재도전 큐(`ALT-01-04`)가 빈 목록이 된다.** ACTIVE 정책 중 나이 하한이 아직 안 찬 것이 이 정책 하나뿐이었다. 기능은 그대로고, 분리지급 규칙(`V36`, `FCT-209` 하한 19세)이 검수를 거쳐 ACTIVE 로 올라가면 다시 채워진다.
